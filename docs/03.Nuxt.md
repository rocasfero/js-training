...
その一つが [Vue](https://vuejs.org/) で、上記の Todo アプリを Vue で書き替えると下記の通り。
...
あとはプロパティ(Model)の変化に応じて検知して見た目に自動的に反映(ViewModel)される、という点が特筆すべき点です。
