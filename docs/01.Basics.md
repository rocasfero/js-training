...
fs.readFile('a_huge_file.txt', (err, data) => {
  // do something ...
...
### Promise[ES6]
前述のコールバック地獄を抜け出すための手段として ES6 で実装された Promise というものがある（thenを呼び出せるオブジェクトを "thenable" と呼んだりする）。
...
// Promiseオブジェクトは then で繋げられて、直前のPromise/Thenableが解決されないと実行されない。
promise.then(() => {
  console.log('やったぜ！！！！');
});
```

⇒ resolve / reject に値を渡すと、次の then のコールバック関数内で使うことができる。次に使いたい値を return していけば良い。
```javascript
...
⇒ 非同期処理を重ねたい場合には、はじめに非同期関数を Promise でラップして、それを return すれば良い。
```javascript
